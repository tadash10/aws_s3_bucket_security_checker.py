...
class S3BucketManager:
    ...
    def disable_bucket_access(self, bucket_name):
        try:
            ...
            self.s3.put_bucket_acl(Bucket=bucket_name, AccessControlPolicy={'Grants': []})
            return True
        except ClientError as e:
...
def main():
    ...
    insecure_buckets = [bucket['Name'] for bucket in bucket_manager.list_buckets() if
                        bucket_manager.is_bucket_insecure(bucket_manager.get_bucket_acl(bucket['Name']).get('Grants', []))]
    secure_buckets = [bucket['Name'] for bucket in bucket_manager.list_buckets() if
